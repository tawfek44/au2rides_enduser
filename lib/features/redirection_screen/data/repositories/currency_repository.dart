...
class CurrencyRepositoryImpl extends CurrencyRepository{
  ...
  clearCurrenciesFromLocalDB({required String tableName}) async{
    final response = await _currencyDataSource.deleteAllCurrencyInDatabase(tableName: tableName);
    return response;
  }

  @override
  getCurrencies({required String lang}) async {
    final response = await _currencyDataSource.getAllCurrencyFromNetworkDatabase(lang: lang);
    return response;
  }
