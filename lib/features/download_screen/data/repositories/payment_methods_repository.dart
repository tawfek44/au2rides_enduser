...
class PaymentMethodsRepositoryImpl extends PaymentMethodsRepository {
  ...
  @override
  clearPaymentMethodsDataInLocalDB({required tableName}) async {
    final response = await _paymentMethodsDataSource
        .clearAllPaymentMethodsDataFromLocalDb(tableName: tableName);
    return response;
  }

  @override
  downloadPaymentMethodsFromNetworkDB({required tableDefinitions, required appLang}) async {
    final response =
        await _paymentMethodsDataSource.downloadPaymentMethodsDataFromNetworkDb(tableDefinitions: tableDefinitions,appLang: appLang);
    return response;
  }
  ...
  savePaymentMethodsDataInLocalDB({required values, required tableName}) async {
    final response =
        await _paymentMethodsDataSource.savePaymentMethodsInLocalDb(tableName: tableName,values: values);
    return response;
  }
}
